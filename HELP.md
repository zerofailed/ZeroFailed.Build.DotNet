...
| `DotNetTestFileLoggerProps`        | see below                                                                  |                                  | Allow the file logger properties used when running tests via 'dotnet test' to be customised. Defaults to 'DotNetTestFileLoggerProps_VSTest' or 'DotNetTestFileLoggerProps_MTP', depending on the test platform detected. Supports lazy evaluation.                                                                 |
| `DotNetTestFileLoggerProps_MTP`    | see below                                                                  |                                  | The diagnostic logging arguments passed to 'dotnet test' when using the Microsoft Testing Platform. Defaults to enabling the MTP diagnostic file logger, with a verbosity derived from 'DotNetFileLoggerVerbosity' and the '.diag' files written to the repo root. Supports lazy evaluation.                       |
| `DotNetTestFileLoggerProps_VSTest` | { "/flp:verbosity=$DotNetFileLoggerVerbosity;logfile=$DotNetTestLogFile" } |                                  | The file logger properties passed to 'dotnet test' when using the VSTest platform. Defaults to "/flp:verbosity=<DotNetFileLoggerVerbosity>;logfile=<DotNetTestLogFile>". Supports lazy evaluation.                                                                                                                 |
...
| `DotNetTestResultsDir`             | ""                                                                         | ZF_BUILD_DOTNET_TEST_RESULTS_DIR | The directory where 'dotnet test' result (.trx) files are written. When empty (the default), each test project writes to its own 'TestResults' folder next to its build output. Set to a path (relative paths are resolved against the repo root) to collect every project's results in a single shared directory. |
...
The default `DotNetTestFileLoggerProps` value depends on the test platform detected for the solution:

- **VSTest**: `DotNetTestFileLoggerProps_VSTest` configures the MSBuild file logger, writing to `DotNetTestLogFile`.
- **Microsoft Testing Platform (MTP)**: `DotNetTestFileLoggerProps_MTP` enables the MTP diagnostic file logger, mapping `DotNetFileLoggerVerbosity` to the equivalent MTP verbosity (`Critical`, `Error`, `Warning`, `Information` or `Trace`) and writing the `.diag` files to the repo root. Only options supported by both MTP 1.x and 2.x hosts are used, so no log file prefix is set (that option was renamed in MTP 2.0) and MTP's default file naming applies (e.g. `<asm>_<tfm>_<arch>_<timestamp>.diag` on MTP 2.x).
...
| `TruncateOversizedCoverageReportThreshold` | 60000                     |                                                  | The threshold for Markdown code coverage reports (as a character count), above which it will be truncated. Defaults to 60000 characters. |
| `UseGitHubFlavour`                         | $env:GITHUB_ACTIONS -eq 'true' | ZF_BUILD_DOTNET_USE_GITHUB_FLAVOUR               | When true, the Markdown code coverage summary is generated in the GitHub-optimised format ('SummaryGithub.md'), which is the file the shared 'Add Code Coverage PR comment' action looks for. Defaults to true when running on GitHub Actions. |
