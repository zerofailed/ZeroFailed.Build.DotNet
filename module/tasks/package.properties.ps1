...
$SkipNuGetPackages ??= [Convert]::ToBoolean((property ZF_BUILD_DOTNET_SKIP_NUGET_PACKAGES $false))

# Synopsis: When true, no NuSpec-based NuGet packages will be built, meaning 'dotnet pack' will not be run for them.
$SkipNuspecPackages ??= [Convert]::ToBoolean((property ZF_BUILD_DOTNET_SKIP_NUSPEC_PACKAGES $false))
...
$SkipProjectPublishPackages ??= [Convert]::ToBoolean((property ZF_BUILD_DOTNET_SKIP_PROJECT_PUBLISH_PACKAGES $false))
